...
class RegionSelect extends Component {
	...
	onComponentMouseTouchDown (event) {
		if (event.button != 0 || event.target.dataset.wrapper || event.target.dataset.dir || isSubElement(event.target, (el) => el.dataset && el.dataset.wrapper)) {
			return;
		}
	...
	render () {
		...
		return (
			<div
				ref='image'
				style={objectAssign({}, style.RegionSelect, this.props.style, {position: 'absolute'}, {left: 0} )}
